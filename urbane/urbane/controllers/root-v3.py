...
def validate(params, update=False):
    ...
    #Added for validating the domain
    field = 'domain'
    not_empty()
...
def check_region():
    ...
    log.info('region1')
    region = request.GET.get('region', request.GET.get('region_name', request.POST.get('region', request.POST.get('region_name', 'RegionOne'))))
    log.info(region)
    log.info('++++++++++')
    log.info(conf.get('region'))
    if conf.get('region', 'RegionOne') != region:
        abort(403)
    log.info('before return from region check')
    return True

# keystone v2 service catalog
v2_service_catalog = None

#Keystone v3 service_catalog
...
def authenticate():
    ...
    log.info('Inside authenticateeeeeddddddddd')
    log.info('Inside authenticate function of api')
    check_region()
    log.info('iiiafter region check')
    ...
    if auth_token:
        ...
        log.info('Eiiiiintering since auth_token is present')
        log.info('auth token is ')
        log.info(auth_token)
    ...
    # issue new admin auth token and check roles
        ...
        log.info('Entering since auth token is not present')
        # KS API v2
        if KS_API_V == 2:
            res = req.post(
                auth_url + '/tokens',
        ...
        # KS API v3
            ...
            res = req.post(
                auth_url + '/auth/tokens', json, headers
            )
...
class RootController(RestController):
    ...
    def get_all(self, **params):
        log.info('INSIDEEEEEE GET ALLLLLLLL')
    ...
    def deploy(self, id=None, signup=None, **params):
        ...
        if not type(signup) is Signup:
            if id:
                ...
                log.info('inside else part of the loop')
                abort(400)
        ...
        if KS_API_V == 2:
            ...
            #TODO
            try:
                endpoint_url = "/tenants/%s/users/%s/roles/OS-KSADM/f718b444a5ea478a886803e7bdb2bdb8" % (tenant['id'], user['id'])
            ...
            #auth_url = 'http://192.168.122.147:5000/v3'
            auth_url = 'https://api.stage2.nephoscale.net:5000/v3'
            log.info('V3 SECTION STARTS HERREEE')
            ...
            #TODO
            try:
                endpoint_url = "/projects/%s/users/%s/roles/f718b444a5ea478a886803e7bdb2bdb8" % (project['id'], user['id'])
