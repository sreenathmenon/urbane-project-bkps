...
def _json_encode_default_(d):
    if hasattr(d, 'isoformat'): # is datetime or date or time?
        return datetime.strftime(d, conf.datetime_store_format)
    else:
        return None
...
def json_encode(o):
    ...
    print o
    print '+++++++++++++++++++++++++'
    return json.dumps(o, default=_json_encode_default_)

json_decode = json.loads


# values normalization helper
def normalize(params):
    _params_ = {}
    for k in params:
        # remove trailing and doubled spaces
        if hasattr(params[k], 'strip'):
            _params_[k] = re.sub(r'\s+', params[k].strip(), ' ')
        else:
            _params_[k] = params[k]
    _params_.pop('format', None)
    return _params_

# returns credit card type by its number
def get_cc_type(number):
    cc = number.replace(' ', '')
    if re.match(r'^3[47]\d{13}$', cc):
        return 'AX'
    elif re.match(r'^5[1-5]\d{14}$', cc):
        return 'MC'
    elif re.match(r'^4\d{12,15}$', cc):
        return 'VS'
    else:
        return None

def format_cc_number(number, mangle=True, prefix=True):
    if mangle:
        if prefix:
            return '%s: **** **** **** %s' % (get_cc_type(number) or 'Unknown', number[-4:])
        else:
            return '**** **** **** %s' % number[-4:]
    else:
        if prefix:
            return '%s: %s' % (get_cc_type(number) or 'Unknown', number)
        else:
            return number
